...
import { playTap, playSuccess, playNotification } from '../utils/audio';
                ...
                <tr key={inv.InvoiceID} className="hover:bg-slate-800/40 transition-colors">
                  ...
                  <td className="p-4">
                    <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[10px] uppercase tracking-wider font-bold border ${
                      inv.Status === 'Paid' ? 'bg-emerald-500/10 border-emerald-500/25 text-emerald-400' :
                  ...
                  <td className="p-4 text-center">
                    <div className="flex items-center justify-center gap-2">
                      ...
                      <button
                        onClick={() => { playTap(); setSelectedInvoiceForReceipt(inv); }}
                      ...
                      <button
                        onClick={async () => {
                          playTap();
                          const s = students.find(x => x.StudentID === inv.StudentID);
                          const p = payments.find(x => x.InvoiceID === inv.InvoiceID);
                          const v = s ? vehicles.find(x => x.VehicleID === s.VehicleID) : undefined;
                          const qr = getQrCodeUrl(inv, s);
                          await downloadInvoicePdf(inv, s, p, v, qr);
                          playSuccess();
                        }}
                        className="p-2 hover:bg-slate-800 text-amber-400 hover:text-amber-300 rounded-lg transition-colors cursor-pointer"
              ...
              <div className="flex items-center justify-between p-4 bg-slate-950 text-white no-print">
                ...
                <div className="flex items-center gap-2">
                  <button 
                    onClick={async () => {
                      playTap();
                      const qr = getQrCodeUrl(selectedInvoiceForReceipt, student);
                      await downloadInvoicePdf(selectedInvoiceForReceipt, student, paymentRecord, vehicle || undefined, qr);
                      playSuccess();
                    }}
                    className="bg-emerald-500 hover:bg-emerald-600 text-slate-950 font-extrabold px-3.5 py-1.5 rounded-lg text-xs inline-flex items-center gap-1.5 transition-colors cursor-pointer shadow-md"
                    title="Download high-fidelity PDF file"
                  >
