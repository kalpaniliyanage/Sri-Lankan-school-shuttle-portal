    ...
    <div className="bg-slate-900 border border-slate-800/80 rounded-2xl shadow-xl p-6" id="reports-center-card">
        ...
        <div className="flex-1 space-y-1.5">
          ...
          <div className="grid grid-cols-3 gap-2 bg-slate-900 p-1 rounded-xl border border-slate-800">
            <button
              ...
              className={`px-3 py-2 text-xs font-bold rounded-lg text-center cursor-pointer transition-all ${
                reportType === 'defaulters' ? 'bg-amber-400 text-slate-950 shadow-sm' : 'text-slate-400 hover:text-white'
              }`}
            >
            ...
            <button
              ...
              className={`px-3 py-2 text-xs font-bold rounded-lg text-center cursor-pointer transition-all ${
                reportType === 'route_revenue' ? 'bg-amber-400 text-slate-950 shadow-sm' : 'text-slate-400 hover:text-white'
              }`}
            >
              Route Revenue
            </button>
            <button
              ...
              className={`px-3 py-2 text-xs font-bold rounded-lg text-center cursor-pointer transition-all ${
                reportType === 'school_totals' ? 'bg-amber-400 text-slate-950 shadow-sm' : 'text-slate-400 hover:text-white'
              }`}
            >
              School Summaries
            </button>
      ...
      <div className="border border-slate-800/80 rounded-2xl p-6 bg-slate-950/20" id="printable-report-area">
        ...
        <div className="hidden print:block text-center space-y-1 pb-6 mb-6 border-b border-dashed border-slate-800">
          <h2 className="text-xl font-bold uppercase text-slate-100">School Transport Authority</h2>
                  ...
                  {defaultersData.map((d, index) => (
                    <tr key={index} className="hover:bg-slate-900/40 transition-colors">
                      ...
                      <td className="p-3.5 pr-4 text-center">
                        <span className={`px-2.5 py-1 font-bold text-[9px] uppercase tracking-wider rounded-full border ${
                          d.Status === 'Overdue' ? 'bg-rose-500/10 border-rose-500/25 text-rose-400' : 'bg-amber-500/10 border-amber-500/25 text-amber-400'
                        }`}>
                          {d.Status}
                        </span>
                      </td>
